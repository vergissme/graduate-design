...
public class ImageWaterMarkUtil {
     ...
     * @return void
     */
    public static BufferedImage addWaterMark(String sourceImgPath, String waterMarkContent, String watermarkPath,
                                             String fileExt, Integer degreeAngle, float transparency, boolean isText){
        long startTime = System.currentTimeMillis();
        // 加水印
    ...
    private static void addFullImgWaterMarkMethod(Graphics2D g,String watermarkPath, int srcImgWidth, int srcImgHeight) throws Exception{
        ...
        int rowsNumber = getRowNumberByWaterMarkImg(srcImgHeight,height);
        int columnsNumber =getColumnsNumberByWaterMarkImg(srcImgWidth,width);
        for(int j=0;j<=rowsNumber;j++){
            for(int i=0;i<=columnsNumber;i++){
                ...
                // 表示水印图片的位置
                if(i==0){
                    g.drawImage(watermarkImg, 0, y,null);
                }else{
    ...
    private static void addTextWaterMarkMethod(Graphics2D g,Font font,String waterMarkContent,int srcImgWidth,int srcImgHeight) {
        ...
        for(int j=0;j<=rowsNumber;j++){
            for(int i=0;i<=columnsNumber;i++){
                ...
                // 表示水印图片的位置
                if(i==0){
                    g.drawString(waterMarkContent, 0, y);
                }else{
                    x = x+(i*width);
     ...
     * @param height 原始的水印图片高度
     * @return int
     **/
    private static int getRowNumberByWaterMarkImg(int srcImgHeight, int height) {
        //图片的高  除以  水印图片高度  打印的行数(以水印的高为间隔)
        int rowsNumber = srcImgHeight/height;
        //防止图片太小而水印太长，所以至少打印一次
        if(rowsNumber < 1){
            rowsNumber = 1;
        }
        return rowsNumber;
     ...
     * @return int
     **/
    private static int getColumnsNumberByWaterMarkImg(int srcImgWidth, int width) {
        //图片的宽 除以 水印图片的宽  每行打印的列数(以水印图片的宽为间隔)
        int columnsNumber = srcImgWidth/width;
        //防止图片太小而水印图片太长，所以至少打印一次
        if(columnsNumber < 1){
            columnsNumber = 1;
        }
